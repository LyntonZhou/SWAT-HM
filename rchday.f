...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
 ...
 !       pdvar(46) = rchdy(60,j)   ! water temperature deg c
        ...
        !! added by Zhou 20190306
         pdvar(43) = hmlrchdy(9,1,j)   
         pdvar(44) = hmlrchdy(11,1,j)
         pdvar(45) = hmlrchdy(12,1,j)
         pdvar(46) = hmlrchdy(9,1,j) 
     &   + hmlrchdy(11,1,j) + hmlrchdy(12,1,j)
...
!!    added for binary files 3/25/09 gsm line below and write (77777
     ...
     &          iyr, rch_dakm(j),(pdvr(ii), ii = 1, itotr), iyr
          ...
          !! Write daily H.metal in channel
          if (hml_eqn == 0) then
		   write(307,5003) j, iyr, iida, rch_dakm(j),
     &             hmlrchdy(1,1:mhml-1,j),
     &             hmlrchdy(2,1:mhml-1,j),              
     &             hmlrchdy(3,1:mhml-1,j),               
     &             hmlrchdy(4,1:mhml-1,j),               
     &             hmlrchdy(9,1:mhml-1,j),   
     &             hmlrchdy(10,1:mhml-1,j),
     &             hmlrchdy(11,1:mhml-1,j),
     &             hmlrchdy(12,1:mhml-1,j),
     &             hmlrchdy(5,1:mhml-1,j),
     &             hmlrchdy(6,1:mhml-1,j),
     &             hmlrchdy(7,1:mhml-1,j),
     &             hmlrchdy(8,1:mhml-1,j),
     &             hmlrchdy(13,1:mhml-1,j),               
...
!!    added for binary files 3/25/09 gsm line below and write (77777
     ...
     &              iyr, rch_dakm(j), (pdvar(ii), ii = 1, 45)
        ...
        !! added by Zhou 20150723
        write(307,5003) j, subgis(j), iida, rchdy(2,j),
     &		sum(hmlrchdy(1:4, 1:mhml-1,j),dim=1),                     
     &		sum(hmlrchdy(9:12,1:mhml-1,j),dim=1),                     
     &        sum(hmlrchdy(1:2,1:mhml-1,j),dim=1),                     
     &        sum(hmlrchdy(3:4,1:mhml-1,j),dim=1),                      
     &        sum(hmlrchdy(9:10,1:mhml-1,j),dim=1),                     
     &        sum(hmlrchdy(11:12,1:mhml-1,j),dim=1)
 ...
 5000 format ('REACH ',i4,1x,i8,1x,i5,1x,47e12.4,i6)
