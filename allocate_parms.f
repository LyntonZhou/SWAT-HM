...
!!    mhml        |none          |max number of heavy metals inputted in wshed
!!    mhmldb      |none          |max number of heavy metals in metal.dat
...
! Allocate variables for heavy metals in channels
	...
	allocate (sedhml_conc(4,mhml,mch))
      allocate (chhml_mix(mhml,mch))
      allocate (chhml_rsp(mhml,mch))
      allocate (chhml_stl(mhml,mch))
	allocate (sedhml_bry(mhml,mch))
! Allocate variables for heavy metals in reserviors
	allocate (lkhml_conc(4,mhml,mres))
	allocate (lksedhml_conc(4,mhml,mch))
      allocate (lkhml_mix(mres))
      allocate (lkhml_rsp(mres))
      allocate (lkhml_stl(mres))
      allocate (lkshml_act(mres))
	allocate (lksedhml_bry(mres))

	allocate(hml_sol_o(mhml))
	allocate(hml_exch_o(mhml))
	allocate(hml_lig_o(mhml))
	allocate(hml_nlab_o(mhml))

	allocate(hhhml_sol_o(mhml,24))
	allocate(hhhml_exch_o(mhml,24))
	allocate(hhhml_lig_o(mhml,24))
	allocate(hhhml_nlab_o(mhml,24))

	allocate(setlhml_nlab(mhml))
	allocate(setlhml_exch(mhml))
	allocate(rsushml_nlab(mhml))
	allocate(rsushml_exch(mhml))
	allocate(bury_nlab(mhml))
	allocate(bury_exch(mhml))
	allocate(difus_hml(mhml))


! End of heavy metals' allocations that are in channels
!!     allocate (chside(mch))
!!    allocate (dep_chan(mch))
!!    allocate (disolvp(mch))
!!    allocate (drift(mch))
...
!!    allocate (organicp(mch))
!!    allocate (orig_sedpstconc(mch))
! Allocation for heavy metals
	allocate (orig_sedhmlconc(4,mhml,mch))
	allocate (frac_rch_cbn_soluble(mhml,mch))
! Allocate variables for heavy metals in channels
      allocate (sedhml_act(mch))
      allocate (sedhml_rea(mhml,mch))
! End of heavy metals' allocations that are in channels
...
!!    reach output for h.metal
	allocate (hmlrchdy(20,mhml,msub)) 
      allocate (hmlrchmono(20,mhml,msub)) 
      allocate (hmlrchyro(20,mhml,msub)) 
...
!!    arrays which contain data related to subbasins
      ...
      !! added by Zhou 20190110
      allocate (hml_dep_total(mhru))
      allocate (hml_dep_frac(mhru))
      allocate (weth_amt(mhru))
...
! Add variables for heavy metals, for each h.metal
	allocate (sub_hml_sol(mhml,msub,4))
      allocate (sub_hml_lig(mhml,msub))
	allocate (sub_hml_exch(mhml,msub))
      allocate (sub_hml_nlab(mhml,msub))
...
!    Drainmod tile equations  01/2006
      ...
      !! added by Zhou 20150728 
      allocate (frac_sol_cbn_soluble(mlyr,mhru))
...
!!    arrays which contain data related to soil layers, HRUs, heavy metals
      allocate (orig_solhml_sol(mhml,mhru,mlyr))
	allocate (orig_solhml_lig(mhml,mhru,mlyr))
	allocate (orig_solhml_exch(mhml,mhru,mlyr))
	allocate (orig_solhml_nlab(mhml,mhru,mlyr))
      allocate (sol_hml_sol(mhml,mhru,mlyr+1))
	allocate (sol_hml_lig(mhml,mhru,mlyr+1))
	allocate (sol_hml_lab(mhml,mhru,mlyr))
	allocate (sol_hml_nlab(mhml,mhru,mlyr))
...
!! Add those for heavy metals
	allocate (lkhml_mass(mres))
      allocate (lkshml_mass(mres))
...
!! Add those for heavy metals
      allocate (orig_lkhmlconc(4,mhml,mres))
!! allocate (orig_lkshmlconc(mres))
	allocate (orig_lksedhmlconc(2,mhml,mres))
	allocate (frac_res_cbn_soluble(mhml,mres))
...
!!    arrays which contain data related to heavy metals in database
	allocate (hmlwashout_ef(mhmldb))
	allocate (sol_hmlkp(mhmldb,3))
	allocate (sol_hmlkx(mhmldb))
	allocate (sol_hmlkm(mhmldb))
	allocate (wtr_hmlkl(mhmldb))
	allocate (wtr_hmlkr(mhmldb))
	allocate (point_gamma(mhmldb))
	allocate (plt_hmlku(mhmldb))
      allocate (hml_wsol(mhmldb))
      allocate (hml_weth(mhmldb))
	allocate (i_hml_no(mpdb))
...
! Add depth for heavy metals in soil
	allocate (hml_dep(mnr,mapp,mhru))
...
!!    allocate basin-channel routing array for h.metal
	allocate (hmlroute(4,mhml,mhyd))
	allocate (hhhmlroute(4,mhml,mhyd,24))
...
! HRU heavy metal mobilized
	allocate (hruhml(mhru))
...
!    Drainmod tile equations  01/2006 
      ...
      ! added by Zhou 20190306
      allocate (plt_hml(mhru))
      allocate (hml_plt(mhru))
...
!!     added by Zhou 20150728    
!!    arrays which contain data related to heavy metals, HRUs
      allocate (hml_enr(mhml,mhru))
	allocate (hml_fr(mhml,mhru))
...
!!    arrays which contain data related to heavy metals
      allocate (hml_lat(mhml,mhru))
      allocate (n_hml_no(mhml))
      allocate (hml_prk(mhml,mhru))
      allocate (weth_hml(mhml))
      allocate (wshd_hmldg(mhml))
      allocate (atmo_hml(mhml))
...
! Add heavy metals for reccnst
	allocate (solhmlcnst(mrecc))
      allocate (srbhmlcnst(mrecc))
...
! Add heavy metals for reccnst summed as annual values
      allocate (solhmlyr(mrecy,myr))
      allocate (srbhmlyr(mrecy,myr))
...
! Add heavy metals for reccnst summed as monthly values
      allocate (solhmlmon(mrecm,myr,12))
      allocate (srbhmlmon(mrecm,myr,12))
...
!! Add heavy metals for out flow summary
      allocate (whmlaao(mhml,5))
      allocate (whmlmono(mhml,5))
      allocate (whmlyro(mhml,5))
      allocate (whmldayo(mhml,5))

!!H.metal upward transported h.metal,summed in watershed    
	allocate (shmlup(mhml))
