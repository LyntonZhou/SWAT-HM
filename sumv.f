...
!!                               |contribution to stream from HRU on day
!!                               |(in solution)
!!    hruhmld(:,1,:)|kg hml      |amount of h.metal type in surface runoff
!!                               |contribution to stream from HRU on day
!!                               |(in solution)
!!    hruhmld(:,2,:)|kg hml      |amount of h.metal type in surface runoff
!!                               |contribution to stream from HRU on day
!!                               |(sorbed to sediment)
!!    hruhmld(:,3,:)|kg hml      |total h.metal leached from soil profile 
!!    hruhmld(:,4,:)|kg hml      |amount of h.metal type in lateral flow
!!                               |contribution to stream from HRU on day
!!                               |(in solution)
!!    hruhmld(:,5,:)|kg hml      |amount of h.metal type in surface flow
!!                               |contribution to stream from HRU on day
!!                               |, as  labile species in mobilized sediment
!!    hruhmld(:,6,:)|kg hml      |amount of h.metal type in surface flow
!!                               |contribution to stream from HRU on day
!!    hruhmld(:,7,:)|kg hml      |amount of h.metal from weathing
!!                               |, as non-labile species in mobilized sediment
     ...
     &                                                              1.e6
      ...
      !! h.metal summary
        if (hruhml(j) == 1) then
          do k = 1, n_hml_mx
            !! HRU summary
		  ! Hml in surface aqueous flow 
            hruhmld(k,1,j) = hml_surq(k,j) * 1. * hru_ha(j)
            ! Hml in lateral flow 
            hruhmld(k,2,j) = hml_lat(k,j) * 1. * hru_ha(j)
		  ! Hml leached from soil profile 	
		  ...
		  hruhmld(k,9,j) = hml_agr_total(j)/365. * 1. * hru_ha(j)
            ...
            whmldayo(k,2) = whmldayo(k,2)
     &      + sum(hml_sed(:,k,j),dim=1) * hru_dafr(j)
	      ! leached from the bottom of soil layer
            whmldayo(k,3) = whmldayo(k,3) + hml_prk(k,j) * hru_dafr(j)
            ! leached through lateral flow
            whmldayo(k,4) = whmldayo(k,4) + hml_lat(k,j) * hru_dafr(j)
          end do
        end if
