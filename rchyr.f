...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
 ...
 !! added NO3 Concentration to output.rch (for daily only) gsm 10/26/2011
         ...
         pdvar(43) = hmlrchyro(9,1,j)   
         pdvar(44) = hmlrchyro(11,1,j)
         pdvar(45) = hmlrchyro(12,1,j)
         pdvar(46) = hmlrchyro(9,1,j) 
     &   + hmlrchyro(11,1,j) + hmlrchyro(12,1,j)       
     ...
     &                             (pdvr(ii), ii = 1, itotr), iyr 
          ...
          !! Write monthly H.metal in channel
          if (hml_eqn == 0) then
          write(307,5003) j, iyr, iyr, rch_dakm(j),
     &             hmlrchyro(1,1:mhml-1,j),
     &             hmlrchyro(2,1:mhml-1,j),              
     &             hmlrchyro(3,1:mhml-1,j),               
     &             hmlrchyro(4,1:mhml-1,j),               
     &             hmlrchyro(9,1:mhml-1,j),   
     &             hmlrchyro(10,1:mhml-1,j),
     &             hmlrchyro(11,1:mhml-1,j),
     &             hmlrchyro(12,1:mhml-1,j),
     &             hmlrchyro(5,1:mhml-1,j),
     &             hmlrchyro(6,1:mhml-1,j),
     &             hmlrchyro(7,1:mhml-1,j),
     &             hmlrchyro(8,1:mhml-1,j),
     &             hmlrchyro(13,1:mhml-1,j)/Real(idlast),               
