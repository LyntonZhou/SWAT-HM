...
!! septic variables for output.std
      ...
      !! added by Zhou 20161030
      real :: sedrchin, sedrchout
...
! output files 
! output files for h.metals added by Zhou 2015/5/1
	real, dimension (:,:,:), allocatable :: hmlrchdy, hmlrchmono, hmlrchyro
...
!    Drainmod tile equations  01/2006
      ...
      !! added by zhou 20190109
      real, dimension (:), allocatable :: hml_dep_total, hml_dep_frac
      real, dimension (:), allocatable :: plt_hml, hml_plt
      real, dimension (:), allocatable :: weth_amt
...
!! added by Zhou 2015/7/1
      character(len=10), dimension (300) :: hmlname
...
!! 'hml' is an abbreviation of heavy metal
!! Appended by Zhou 2015/7/1
	real :: resushml,setlhml
	integer :: mhml,mhmldb
	integer, dimension (:), allocatable :: n_hml_no,i_hml_no
	real, dimension (:,:), allocatable :: whmldayo,whmlmono,whmlyro
	real, dimension (:,:), allocatable :: whmlaao
	real, dimension (:,:,:), allocatable :: hruhmld,hruhmla,hruhmlm
	real, dimension (:,:,:), allocatable :: hruhmly
	real, dimension (:,:,:), allocatable :: chhml_conc,sedhml_conc
	real, dimension (:,:), allocatable :: chhml_mix,chhml_rea
	real, dimension (:,:), allocatable :: chhml_stl,chhml_rsp
	real, dimension (:,:), allocatable :: sedhml_rea,sedhml_bry
	real, dimension (:,:,:), allocatable :: lkhml_conc,lksedhml_conc
	real, dimension (:), allocatable :: sedhml_act
	real, dimension (:,:,:), allocatable :: orig_sedhmlconc
	real, dimension (:,:,:), allocatable :: sub_hml_sol
	real, dimension (:,:), allocatable :: sub_hml_lig
	real, dimension (:,:), allocatable :: sub_hml_exch
	real, dimension (:,:), allocatable :: sub_hml_nlab
...
! Soluable
	...
	real, dimension (:,:,:), allocatable :: orig_solhml_sol 
! Ligand-fixed
	real, dimension (:,:,:), allocatable :: sol_hml_lig 
	real, dimension (:,:,:), allocatable :: orig_solhml_lig 
!  labile in soil
	real, dimension (:,:,:), allocatable :: sol_hml_lab 
	real, dimension (:,:,:), allocatable :: orig_solhml_exch
! Non-labile in soil
	real, dimension (:,:,:), allocatable :: sol_hml_nlab 
	real, dimension (:,:,:), allocatable :: orig_solhml_nlab
	real, dimension (:), allocatable :: lkhml_rea
	real, dimension (:), allocatable :: lkhml_kd
	real, dimension (:), allocatable :: lkhml_stl,lkhml_rsp
	real, dimension (:), allocatable :: lkhml_mix,lksedhml_bry
	real, dimension (:), allocatable :: lkshml_conc,lkshml_rea
	real, dimension (:), allocatable :: lkshml_bry,lkshml_act
	real, dimension (:,:,:), allocatable :: orig_lkhmlconc
	real, dimension (:,:,:), allocatable :: orig_lksedhmlconc
	real, dimension (:), allocatable :: hml_wsol
      real, dimension (:), allocatable :: hml_weth
	integer, dimension (:), allocatable :: hmlflg
	...
	real, dimension (:,:), allocatable :: hml_fr, hml_enr
	real, dimension (:,:), allocatable :: hml_rock
	real, dimension (:,:,:), allocatable :: hml_sed
	real, dimension (:,:,:), allocatable :: hml_lag
	real, dimension (:), allocatable :: weth_hml,wshd_hmldg
      real, dimension (:), allocatable :: atmo_hml
	real, dimension (:,:,:), allocatable :: solhmlmon,srbhmlmon
	real, dimension (:,:), allocatable :: solhmlyr,srbhmlyr
	real, dimension (:), allocatable :: solhmlcnst,srbhmlcnst
	real, dimension (:), allocatable :: hsolhml,hsorhml
...
! Solubility of metal in channel
	...
	integer, dimension (:,:,:), allocatable :: ihml
	integer, dimension (:), allocatable :: hruhml
	integer, dimension (:), allocatable :: nhml
      
      integer :: hml_eqn
!土壤中重金属分配方式，0为三相，1为两相
	real, dimension (:), allocatable :: hmlwashout_ef
! 从矿堆、尾矿等处在降雨情况下溶出的系数
	real, dimension (:,:), allocatable :: sol_hmlkp    
! 可交换态（固相）与液相离子之间的平衡系数(固液分配系数）
	real, dimension (:), allocatable :: sol_hmlkx
! 固相可交换态向半活泼态的转化速率常数
	real, dimension (:), allocatable :: sol_hmlkm
! 固相半活泼态向活泼态的转化速率常数
	real, dimension (:), allocatable :: wtr_hmlkl
! 液相离子与水中有机物浓度形成配位有机物的(速率常数)
	real, dimension (:), allocatable :: wtr_hmlkr
! 水中配位有机物分解为液相离子和配位有机物的(速率常数)
	real, dimension (:), allocatable :: plt_hmlku
! 液相离子的作物摄取速率
	
	real, dimension (:,:), allocatable :: frac_sol_cbn_soluble
! 土壤层有机碳中可溶解配位有机物（如HA、FA等）所得比例
! 如果配位有机物在水相跟固相的平衡常数为Ko的话，
! Ls=Ko[L];Ls*bd*d+SAT*[L]=Ltot
! Ko[L]*bd*d+SAT[L]=Ltot
! [L]=Ltot*1/(Ko*bd*d+SAT)
! frac_sol_cbn_soluble=1/(Ko*bd*d+SAT)
	real, dimension (:), allocatable :: point_gamma
! 重金属与溶解性有机物配位反应时的配位数（1个重金属原子对应的有机物个数）

	real, dimension (:,:,:), allocatable :: hmlroute
	real, dimension (:,:,:,:), allocatable :: hhhmlroute
	real, dimension (:,:), allocatable :: frac_rch_cbn_soluble
! 河流有机碳中可溶性配位有机物（如HA、FA)所占比例
	real, dimension (:,:), allocatable :: frac_res_cbn_soluble
! 湖库有机碳中可溶性配位有机物（如HA、FA)所占比例

	real, dimension (:), allocatable :: hml_sol_o,hml_exch_o
	real, dimension (:), allocatable :: hml_lig_o,hml_nlab_o
	real, dimension (:,:), allocatable :: hhhml_sol_o,hhhml_exch_o
	real, dimension (:,:), allocatable :: hhhml_lig_o,hhhml_nlab_o

	real, dimension (:), allocatable :: setlhml_nlab,setlhml_exch
	real, dimension (:), allocatable :: rsushml_nlab,rsushml_exch
	real, dimension (:), allocatable :: bury_nlab, bury_exch,difus_hml
