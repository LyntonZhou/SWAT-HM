...
1031  format (//6x,' SUB      GIS  MO DA  YR   AREAkm2',27(a10))
      endif
...
!!   added by Zhou 20150723
 !! write headings to heavy metals output file (output.hml)
      if (iprp /= 0) then
        write (33,1000) prog, values(2), values(3),values(1),values(5),
     &                values(6), values(7)
        write (33,1010) title
        write (33,4000)
        write (33,4001) (n_hml_no(j),n_hml_no(j),
     &  n_hml_no(j),n_hml_no(j), j = 1, n_hml_mx)
        write (33,4002) (hmlname(n_hml_no(j)),hmlname(n_hml_no(j)),
     &  hmlname(n_hml_no(j)),hmlname(n_hml_no(j)), j = 1, n_hml_mx)
        write (33,4003) 
      end if
...
!bmp-ri.out
 ...
 4000 format ("Heavy metal output information",/)
 4001 format ("Heavy metal #     ",200(13x,i3,1x))
 4002 format ("Heavy metal name: ",200(a17))
 4003 format (2x,"Please check outhml.rch outhml.sub outhml.hru... ",/)
      end