...
!!    ires_code = 1 turn off reservoirs for santhi
  ...
  !!      call res
        ...
        !! perform reservoir H.metal transformations
        if (hml_eqn == 0) call lakehml
        if (hml_eqn == 1) call lakehml 
      ...
      !! addd by Zhou 20150723
      !! H.metal outflow
	 do k = 1, n_hml_mx
        hmlroute(1,k,ihout) = hml_sol_o(k)  * resflwo
        hmlroute(2,k,ihout) = hml_lig_o(k)  * resflwo
        hmlroute(3,k,ihout) = hml_exch_o(k) * resflwo
        hmlroute(4,k,ihout) = hml_nlab_o(k) * resflwo
	 end do
        ...
        if (ievent > 0) then
          do ii = 1, nstep
           ...
           !! H.metal
		do k = 1, n_hml_mx
		hhhmlroute(1,k,ihout,ii) = hhhml_sol_o(k,ii)  
     &	   	* hhvaroute(2,ihout,ii)
		hhhmlroute(2,k,ihout,ii) = hhhml_lig_o(k,ii)
     &	    * hhvaroute(2,ihout,ii)
		hhhmlroute(3,k,ihout,ii) = hhhml_exch_o(k,ii) 
     &		* hhvaroute(2,ihout,ii)
		hhhmlroute(4,k,ihout,ii) = hhhml_nlab_o(k,ii) 
     &		* hhvaroute(2,ihout,ii)
		end do
