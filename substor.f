...
!!    latq(:)       |mm H2O        |amount of water in lateral flow in HRU for
!!                                 |the day
!!    pst_lag(:,3,:)|kg pst/ha     |amount of pesticide lagged in lateral flow
!!    hml_lag(:,3,:)|kg pst/ha     |amount of h. metal lagged in lateral flow
!!    ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
...
! For h. metal
      if (hruhml(j) == 1) then
        do k = 1, n_hml_mx
          if (hml_lag(k,3,j) < 1.e-6) hml_lag(k,3,j) = 0.0
          hml_lag(k,3,j) = hml_lag(k,3,j) + hml_lat(k,j)
        end do
      end if
...
! For h. metal
      if (hruhml(j) == 1) then
        do k = 1, n_hml_mx
		hml_lat(k,j) = hml_lag(k,3,j) * lat_ttime(j)
	  end do
	end if
