...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      !!===============================    
      integer :: ly
      real :: sumwater, sumwfsc, sumdepth, sat, wc, dp, zlf
...
!     added by Zhou for test
      pdvas(79) = sol_rsd(1,j)
      pdvas(80) = sol_cov(j) 
      pdvas(81) = usle_cfac(j)
      pdvas(82) = usle_mult(j)
      pdvas(83) = cklsp(j)
      pdvas(84) = sol_cbn(1,j)
...
!!    added for binary files 3/25/09 gsm line below and write (33333
     ...
     &      (pdvas(ii), ii = 1, mhruo), iyr
      ...
      !Added by zhou for outhml.hru 20171223
      if (hml_eqn == 0) then
