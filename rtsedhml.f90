    ...
    real :: rsp, stl, sd1, sd2
    real :: sed_por, sol_pd, depnet1
    ...
    if (rtwtr > 0. .and. rchdep > 0.) then
        ...
        if (qdin > 0.01) then
            ...
            tbase = ch_l2(jrch) * 1000. / (3600. * 24. * vc/ prf(jrch))
            ...
            deg = rsp * tbase * ch_w(2,jrch) * ch_l2(jrch) * sol_pd * (1-sed_por)
            dep = stl * tbase * sedin / rchdep
            depnet1 = deg - dep
            
            if (vc < vcrit) depnet = 0.
            if (vc < vcrit) depnet1 = 0.
            
            if (depnet > 1.e-6) then
                
                if (depnet1 > depnet) then
                    sedin = sedin + depnet
                else
                    sedin = sedin + depnet1
                endif
            ...
            !!    Bank erosion
            !!    rchdy(55,jrch) = 0.
            !!    added by Zhou 20160110
            !!    rchdy(3,jrch) = rcharea
            !!    rchdy(4,jrch) = vc
            !!    Channel Degredation
            rchdy(56,jrch) = deg
            !!    Channel Deposition
