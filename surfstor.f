...
!!    pst_surq(:,:) |kg/ha        |amount of pesticide type lost in surface
!!                                |runoff on current day in HRU
!!    hml_lag(:,1,:)|kg pst/ha    |amount of soluble h.metal in surface runoff
!!                                |lagged, including cation and ligand-bound
!!    hml_lag(:,2,:)|kg pst/ha    |amount of sorbed pesticide in surface runoff
!!                                |lagged, including  labile and non-labile
!!    hml_lag(:,3,:)|kg pst/ha    |amount of soluble h.metal in lateral flow
!!    hml_lag(:,4,:)|kg pst/ha    |amount of  labile metal in surface runoff
!!                                |lagged
!!    hml_lag(:,5,:)|kg pst/ha    |amount of non-labile h.metal in surface runoff
!!                                |lagged
!!    hml_sed(:,:)  |kg/ha        |h.metal loading from HRU sorbed onto
!!                                |sediment
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
	...
	! h. metal lagging
      if (hruhml(j) == 1) then
        do k = 1, n_hml_mx
		! For surface runoff, consider free cation and ligand-bound species combinedly
          hml_lag(k,1,j) = hml_lag(k,1,j) + hml_surq(k,j)	
          if (hml_lag(k,1,j) < 1.e-10) hml_lag(k,1,j) = 0.0

		! For sorbed in moblized sediment, consider  labile and non-labiles separately
          hml_lag(k,4,j) = hml_lag(k,4,j) + hml_sed(1,k,j)
	    hml_lag(k,5,j) = hml_lag(k,5,j) + hml_sed(2,k,j)
          if (hml_lag(k,4,j) < 1.e-10) hml_lag(k,4,j) = 0.0
		if (hml_lag(k,5,j) < 1.e-10) hml_lag(k,5,j) = 0.0

	    ! Use hml_lag(k,2,j) to store the total amount of sorbed metal in mobilized sediment
		hml_lag(k,2,j) =hml_lag(k,4,j) + hml_lag(k,5,j)
        end do
      end if
	...
	if (hruhml(j) == 1) then
        do k = 1, n_hml_mx
          hml_surq(k,j) = hml_lag(k,1,j) * brt(j)
          hml_sed(1:2, k,j) = hml_lag(k,4:5,j) * brt(j)
        end do
      end if
