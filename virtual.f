...
!! sum HRU results for subbasin 
     ...
     &                                              hrupstd(irtpest,4,j)
      ...
      !! subbasin averages: h.metal
	!! Handle each h.metal separately
        if (n_hml_mx > 0) then
            do k = 1, n_hml_mx
            !! HRU summary
            ! Hml in surface aqueous flow
            sub_hml_sol(k,sb,1) = sub_hml_sol(k,sb,1) +
     &      hruhmld(k,1,j)
            ! hml in lateral aqueous flow
            sub_hml_sol(k,sb,2) = sub_hml_sol(k,sb,2) +
     &      hruhmld(k,2,j) 
            ! hml leached from soil profile
            sub_hml_sol(k,sb,3) = sub_hml_sol(k,sb,3) +
     &      hruhmld(k,3,j)
            ! Hml uptake by plant
...
!        sub_minps(sb) = sub_minps(sb) / sub_fr(sb)
     ...
     &      sub_tileno3(sb) + sub_gwno3(sb)) * sub_ha          !!surqno3 & latno3 & no3gw
         ...
         !! h.metal route array for each species, every h.metal
	   if (n_hml_mx > 0) then	   
           do k = 1, n_hml_mx
     ...
     &       sub_hml_sol(k,sb,2)
		   ...
		   hmlroute(4,k,ihout) = sub_hml_nlab(k,sb)
             if (sub_hml_sol(k,sb,1) < 0. ) then
                 sub_hml_sol(k,sb,1)= 0.
             end if
             if (sub_hml_sol(k,sb,2) < 0. ) then
                 sub_hml_sol(k,sb,2)= 0.
             end if
             if (sub_hml_sol(k,sb,3) < 0. ) then
                 sub_hml_sol(k,sb,3)= 0.
             end if
             if (sub_hml_sol(k,sb,4) < 0. ) then
                 sub_hml_sol(k,sb,4)= 0.
             end if
          end do
	   end if
     ...
     &                              ratio = sub_hwyld(ii) / sub_wyld(sb)
          ...
          if (hruhml(j) == 1) then
			do k = 1, n_hml_mx
			  hhhmlroute(1,k,ihout,ii) = hmlroute(1,k,ihout) * ratio
			  hhhmlroute(2,k,ihout,ii) = hmlroute(2,k,ihout) * ratio
			  hhhmlroute(3,k,ihout,ii) = hmlroute(3,k,ihout) * ratio
			  hhhmlroute(4,k,ihout,ii) = hmlroute(4,k,ihout) * ratio
			end do
          end if
...
!! added by nbs -- clear with JGA
         ...
         submono(19,sb) = submono(19,sb) + sub_hml_sol(1,sb,1)
