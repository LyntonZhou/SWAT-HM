...
!!    hmlfile		|NA          |HRU soil heavy metal data file name (.hml)
...
!!    hmlnum      |none        |Heavy metal ID number from database file
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      character (len=13) :: hrufile, mgtfile, solfile, chmfile, hmlfile
...
!!    added by Zhou 20150724
      hmlfile(1:10)=hrufile(1:10)
      hmlfile(11:)="hml"
      call caps(hmlfile)
        !! Add heavy metal input specification
        open (32,file=hmlfile)
        eof = 0
        do k = 1, 2
            read (32,6000,iostat=eof) titldum
            if (eof < 0) exit
        end do
        if (eof < 0) exit
        hmlnum = 0
        ...
        if (eof < 0) exit
        if (hmlnum > 0) hmlflg(hmlnum) = 1
        if (eof < 0) exit
        close (32)
	!! End of reading heavy metal input specification
