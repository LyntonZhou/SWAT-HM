...
!!    added by Zhou 20150724
      real ::    eff_hml_up
	real ::    hmlup        
	real ::    Wtr_b, Wtr_a                ! Water at time step i-1 (before) and i (after)
	real ::    Kp			            	! partition constan.
	real ::    HML_b, HML_a				! Total of soluble and  labile species before and after evaporation
      real ::    tmp1, tmp2, tmp3
	real ::    rho, dg
...
!!    added statements for test of real statement above
	...
	effnup = 0.1
      !! added by Zhou 20150724
      eff_hml_up = 0.5	! Do not assign other values. If necessary, its value is whthin (0,1]
...
!! take soil evap from each soil layer
     ...
     &       .00713 * sol_z(ly,j)))
        ...
        if (ly == 2) then
            ...
            dg = sol_z(ly,j) - sol_z(ly-1,j)	! Depth or thickness of current layer
            rho = 1.e3 * sol_bd(ly,j)		! Bulk density of current layer in unit of kg/m3
            do k_i = 1, n_hml_mx
                k = n_hml_no(k_i)	 ! ID No. of metal in db
                kk = i_hml_no(k) ! Sequence No. of input metal, in fact this sequence number is useless
                hmlup = 0.
                ...
                tmp1 = 0.
                tmp1 = (Wtr_a + Kp*rho*dg) / (Wtr_b + Kp*rho*dg)
                HML_a = tmp1**eff_hml_up*HML_b  ! Total of soluble and  labile species after evaporation

                tmp2 = 0.
                tmp2 = Wtr_a/(Wtr_a+Kp*rho*dg)
                
                tmp3 = 0.
                tmp3 = Kp*rho*dg/(Wtr_a+Kp*rho*dg)

                sol_hml_sol(k,j,ly) = HML_a*tmp2
                sol_hml_lab(k,j,ly) = HML_a*tmp3

                hmlup = HML_b-HML_a
                hmlup = Max(hmlup, 0.)
                shmlup(k) = shmlup(k) + hmlup * hru_dafr(j)

                !! Increment of the upper layer soluable species
                sol_hml_sol(k,j,ly-1) = sol_hml_sol(k,j,ly-1) + hmlup
                sol_hml_sol(k,j,ly)   = sol_hml_sol(k,j,ly) - hmlup
                !! Equilibrium would not be striken here, leave it to the end of subbasin calculation
            end do
        endif
      end do
