...
!!    read lake water quality data    
      do 
        ...
        if (eof < 0) exit
        !added by Zhou 20160108
        read (106,1000,iostat=eof) titldum
        if (eof < 0) exit
        read (106,*,iostat=eof) lkhml_stl(i) 
        if (eof < 0) exit
        read (106,*,iostat=eof) lkhml_rsp(i)
        if (eof < 0) exit
        read (106,*,iostat=eof) lkhml_mix(i)
        if (eof < 0) exit
        read (106,*,iostat=eof) lksedhml_bry(i) 
        if (eof < 0) exit
        read (106,*,iostat=eof) lkhml_conc(1,1,i)
        if (eof < 0) exit
        read (106,*,iostat=eof) lksedhml_conc(3,1,i)
        if (eof < 0) exit
        read (106,*,iostat=eof) lksedhml_conc(4,1,i)
        if (eof < 0) exit
        read (106,*,iostat=eof) lkshml_act(i)
        if (eof < 0) exit
